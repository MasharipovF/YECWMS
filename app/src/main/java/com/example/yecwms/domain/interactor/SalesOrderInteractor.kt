...
interface SalesOrderInteractor {
    ...
    suspend fun getSalesOrder(docEntry: Long): Document?
    suspend fun insertSalesOrder(salesOrder: DocumentForPost): Document?
    suspend fun updateSalesOrder(docEntry: Long, salesOrder: DocumentForPost): Boolean
    suspend fun closeSalesOrder(docEntry: Long): Boolean
    var errorMessage: String?
}

class SalesOrderInteractorImpl : SalesOrderInteractor {

    private val repository: SalesOrdersRepository by lazy { SalesOrdersRepositoryImpl() }
    ...
    override suspend fun getSalesOrders(
        ...
        val response = repository.getSalesOrderList(
            filter = filter,
            skipValue = skipValue,
            docStatus = docStatus,
            dateFrom = dateFrom,
    ...
    override suspend fun getSalesOrder(docEntry: Long): Document? {
        val response = repository.getSalesOrder(docEntry)
        return if (response is Document) {
            Log.d("SALESORDER", response.toString())
                ...
                for (sourceItem in resultList) {
                    for (item in itemsResponse.value) {
                        if (item.itemCode == sourceItem.ItemCode && warehouse == sourceItem.WarehouseCode) {
                           ...
                           /*TODO sourceItem.Forma = item.Forma
    ...
    override suspend fun insertSalesOrder(salesOrder: DocumentForPost): Document? {
        val response = repository.insertSalesOrder(salesOrder)
        return if (response is Document) {
            response
        } else {
            errorMessage = (response as ErrorResponse).error.message.value
    ...
    override suspend fun updateSalesOrder(docEntry: Long, salesOrder: DocumentForPost): Boolean {
        val response = repository.updateSalesOrder(docEntry, salesOrder)
    ...
    override suspend fun closeSalesOrder(docEntry: Long): Boolean {
        val response = repository.closeSalesOrder(docEntry)
        return if (response is ErrorResponse) {
            errorMessage = (response).error.message.value
            false
        } else {
